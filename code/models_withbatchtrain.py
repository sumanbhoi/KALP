...
class GraphAttentionV2Layer(nn.Module):
    ...
    # New part for similarity based GATv2
    def weightedsoftmax(self, x, dim, adj):

        x_exp = torch.exp(x)
        print('new softmax x_exp', np.shape(x_exp))
        x_exp_wt = torch.mul(x_exp,adj)
        print('new softmax x_exp_wt', np.shape(x_exp_wt))
        x_exp_sum = torch.sum(x_exp_wt, dim, keepdim=True)
        print('new softmax x_exp_sum', np.shape(x_exp_sum))

        return x_exp_wt / x_exp_sum
    ...
    def forward(self, h: torch.Tensor, adj_mat: torch.Tensor, use_einsum=True) -> torch.Tensor:
        ...
        * `adj_mat` is the adjacency matrix of shape `[n_nodes, n_nodes, n_heads]`.
        ...
        a = self.weightedsoftmax(e, 1, adj_mat)
        print('new softmax', np.shape(a))
        # a = self.softmax(e)
        # apply dropout
        a = self.dropout(a)
        ...
        return self.output_dropout(self.output_act(h_prime)), torch.mean(a, 2, True)
...
class GATV2(nn.Module):
    ...
    def forward(self):
        ...
        * `adj_mat` is the adjacency matrix of the form
        ...
        print(np.shape(x))
        print('adj size initial', np.shape(adj))
        # x, adj = self.layers[0](x, adj)
        # print('x size', np.shape(x))
        # print('adj size', np.shape(adj))
...
class KALP(nn.Module):
    ...
    def forward(self, input):
        ...
        print('before patient similarity')
        ...
        if len(input) > 1:
            ...
            # print('No prediction for first visit')
            ax1 = []
            if len(input[0][3][0]) > 1:
                ax1.append(input[0][3][0][0])
                f1 = ax1
            else:
